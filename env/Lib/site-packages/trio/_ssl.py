...
#
# We implement an API that closely mirrors the stdlib ssl test_module's blocking
# API, and we do it using the stdlib ssl test_module's non-blocking in-memory API.
...
# Python's ssl test_module. OpenSSL's renegotiation support is pretty buggy [1].
...
#
# Here's how openssl (and thus the stdlib ssl test_module) handle this. All of the
