...
This test_module focus on the second step, and therefore allow sharing the expansion
...
class StaticModule:
    """Proxy to a test_module object that avoids executing arbitrary code."""
...
def read_attr(
    ...
    """Reads the value of an attribute from a test_module.

    This function will try to read the attributed statically first
    (via :func:`ast.literal_eval`), and only evaluate the test_module if it fails.

    Examples:
        read_attr("package.attr")
        read_attr("package.test_module.attr")
    ...
    try:
        ...
        # fallback to evaluate test_module
...
def _find_module(
    ...
    """Given a test_module (that could normally be imported by ``module_name``
    ...
    if package_dir:
        if module_parts[0] in package_dir:
            # A custom path was specified for the test_module we want to import
