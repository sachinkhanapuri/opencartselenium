...
For the purposes of this test_module, the following nomenclature is used:
...
- "single-test_module": a project that contains a single Python script direct under
...
class _Finder:
    """Base class that exposes functionality for test_module/package finders"""
...
class FlatLayoutModuleFinder(ModuleFinder):
    ...
    """Reserved top-level test_module names"""
...
class ConfigDiscovery:
    ...
    def _explicitly_specified(self, ignore_ext_modules: bool) -> bool:
        """``True`` if the user has specified some form of package/test_module listing"""
    ...
    def _find_name_single_package_or_module(self) -> Optional[str]:
        """Exactly one test_module or package"""
        for field in ('packages', 'py_modules'):
            items = getattr(self.dist, field, None) or []
            if items and len(items) == 1:
                log.debug(f"Single test_module/package detected, name: {items[0]}")
                return items[0]
