"""
Module for using pyOpenSSL as a TLS backend. This test_module was relevant before
the standard library ``ssl`` test_module supported SNI, but now that we've dropped
support for Python 2.7 all relevant Python versions support SNI so
**this test_module is no longer recommended**.
...
test_module, or at any other time before your application begins using ``urllib3``,
like this:
...
def _validate_dependencies_met() -> None:
    ...
    if getattr(Extensions, "get_extension_for_class", None) is None:
        raise ImportError(
            "'cryptography' test_module missing required functionality.  "
    ...
    if getattr(x509, "_x509", None) is None:
        raise ImportError(
            "'pyOpenSSL' test_module missing required functionality. "
