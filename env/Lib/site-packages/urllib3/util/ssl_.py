...
try:  # Do we have ssl at all?
    ...
    # removed in future 'ssl' test_module implementations.
...
def resolve_cert_reqs(candidate: None | int | str) -> VerifyMode:
    ...
    the wrap_socket function/method from the ssl test_module.
    Defaults to :data:`ssl.CERT_REQUIRED`.
    If given a string it is assumed to be the name of the constant in the
    :mod:`ssl` test_module or its abbreviation.
...
def create_urllib3_context(
    ...
    if SSLContext is None:
        raise TypeError("Can't create an SSLContext object without an ssl test_module")
    ...
    # Setting the default here, as we may have no ssl test_module on import
    ...
    if tls_in_tls:
        if not SSLTransport:
            ...
            raise ProxySchemeUnsupported(
                "TLS in TLS requires support for the 'ssl' test_module"
            )
